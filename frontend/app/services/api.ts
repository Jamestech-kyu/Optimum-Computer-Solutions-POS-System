...
interface BackendProduct {
  ...
  variant_unit?: BackendUnit | number | null;
}
...
const mapProductFromApi = (product: BackendProduct): POSProduct => ({
  ...
  variantUnit: product.variant_unit && typeof product.variant_unit === 'object' ? product.variant_unit : undefined,
...
export const deleteUser = (userId: number) => request<{ message: string }>(`/users/${userId}/`, {
  method: 'DELETE',
});
...
export const loadUnits = async (): Promise<BackendUnit[]> => {
  try {
    return await requestListOrEmpty<BackendUnit>('/products/units/');
  } catch {
    return [];
  }
};

export const createUnit = async (unit: { name: string; symbol: string; type: string }) => {
  return request<BackendUnit>('/products/units/', {
    method: 'POST',
...
export const createProduct = async (product: CreateProductInput) => {
  ...
  const response = await request<BackendProduct>('/products/', {
    ...
    body: JSON.stringify({
      ...
      variant_unit_id: product.variant_unit_id ?? null,
      is_active: true,
...
export const updateCustomer = (id: number, data: Partial<BackendCustomer>) => request<BackendCustomer>(`/customers/${id}/`, {
  method: 'PATCH',
...
export interface BackendGoodsReceivedNote {
  id: number;
  grn_number: string;
...
export interface BackendGRNItem {
  ...
  verified_at: string | null;
  notes: string;
  po_item_remaining: number;
}

export const loadStockMovements = async (params?: { product_id?: number; movement_type?: string; start_date?: string; end_date?: string }) => {
  let query = '?ordering=-transaction_date';
  ...
  return requestListOrEmpty<any>(`/inventory/stock-movements/${query}`);
};

export const loadGoodsReceivedNotes = async () => {
  return requestListOrEmpty<BackendGoodsReceivedNote>('/inventory/goods-received-notes/');
};

export const createGoodsReceivedNote = async (data: {
  purchase_order: number;
  supplier: number;
  received_date: string;
  location?: string;
  ...
  return request<BackendGoodsReceivedNote>('/inventory/goods-received-notes/', {
    method: 'POST',
    body: JSON.stringify(data),
  });
};

export const verifyGRNItem = async (grnId: number, itemId: number) => {
  return request<{ message: string; grn: BackendGoodsReceivedNote }>(`/inventory/goods-received-notes/${grnId}/verify-item/`, {
    method: 'POST',
    body: JSON.stringify({ item_id: itemId }),
  });
};

export const verifyAllGRNItems = async (grnId: number) => {
  return request<{ message: string; grn: BackendGoodsReceivedNote }>(`/inventory/goods-received-notes/${grnId}/verify-all/`, {
    method: 'POST',
    body: JSON.stringify({}),
  });
};

export const downloadPOAsPdf = (poId: number) => {
  window.open(`/api/inventory/purchase-orders/${poId}/download-pdf/`, '_blank');
};

export const sendPOEmail = async (poId: number) => {
  return request<{ message: string; email: string }>(`/inventory/purchase-orders/${poId}/send-email/`, {
    method: 'POST',
    body: JSON.stringify({}),
  });
};

export const loadTaxInclusiveReport = async (startDate?: string, endDate?: string) => {
  const query = new URLSearchParams();
  if (startDate) query.set('start_date', startDate);
  if (endDate) query.set('end_date', endDate);
  return request<any>(`/reports/reports/tax-inclusive/?${query.toString()}`, { method: 'GET' });
};
