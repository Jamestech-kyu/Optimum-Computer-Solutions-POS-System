...
import { approveUser, changePassword, clockInUser, clockOutUser, createCustomer, createProduct, createSupplier, createUnit, deactivateUser, deleteUser, downloadAvailableProducts, downloadProductImportTemplate, hasStoredSession, importProductsFromExcel, loadBackendState, loadUnits, login as apiLogin, logout as apiLogout, receiveAndVerifySupplierInvoice, rejectUser, registerAccount, saveDayBalance, saveSale, saveSupplierInvoice, sendSupplierInvoiceToSupplier, updateCustomer, updateProductStock, updateSupplierInvoicePaymentStatus, updateUser, verifyTwoFactor as apiVerifyTwoFactor } from './services/api';
...
export default function App() {
  ...
  const [units, setUnits] = useState<{ id: number; name: string; symbol: string; type: string }[]>([]);
  ...
  useEffect(() => {
    const closeExpiredDrawer = () => {
      setDayBalance(previousBalance => {
        ...
        window.localStorage.setItem(LAST_DRAWER_CLOSING_BALANCE_KEY, String(previousBalance.openingBalance + cashSalesToday));
  ...
  const finishLocalLogout = () => {
    ...
    setUserRole(null);
    setUserName('');
    setDayBalance(createInitialDayBalance());
    setCompletedSales([]);
    setProducts([]);
  ...
  const handleSupplierOrderCreated = (invoice: Omit<SupplierOrderInvoice, 'id'>) => {
    ...
    const quantityPending = (newInvoice.orderItems || []).reduce((sum, item) => sum + (item.pendingQuantity || 0), 0);
    const isDelivered = quantityPending > 0 ? newInvoice.status === 'pending' : newInvoice.status === 'delivered';
    const deliveredItems = isDelivered
      ? (newInvoice.orderItems || []).filter(item => (item.receivedQuantity ?? item.deliveredQuantity) > 0)
      : [];

    if (deliveredItems.length > 0) {
      setProducts(previousProducts => previousProducts.map(product => {
        const deliveredItem = deliveredItems.find(item => item.productId === product.id);
        const receivedQuantity = deliveredItem?.receivedQuantity ?? deliveredItem?.deliveredQuantity ?? 0;
  ...
  const handleToggleCustomerActive = async (customerId: number, isActive: boolean) => {
    try {
      if (isBackendConnected) {
        await updateCustomer(customerId, { is_active: isActive });
      }
      setCustomers(previous => previous.map(c =>
  ...
  const handleInventoryReceivedAndVerified = (
    invoice: SupplierOrderInvoice,
    items: Array<{ productId: string; productName: string; quantityReceived: number; quantityAccepted: number; quantityRejected: number; rejectionReason: string; unitCost: number }>,
    details: { goodsReceivingNote?: string; deliveryNote?: string; receivingLocation?: string; receivingNotes?: string }
  ) => {
    const deliveredItems = items.filter(item => item.quantityReceived > 0);

    if (deliveredItems.length > 0) {
      setProducts(previousProducts => previousProducts.map(product => {
        const deliveredItem = deliveredItems.find(item => item.productId === product.id);
        return deliveredItem
          ? { ...product, stock: product.stock + deliveredItem.quantityAccepted }
          : product;
      }));
      setStockMovements(previousMovements => [
        ...deliveredItems.map(item => ({
          ...
          reason: `GRN ${details.goodsReceivingNote || invoice.id}`,
    ...
    setSupplierInvoices(previousInvoices => previousInvoices.map(existingInvoice =>
      existingInvoice.id === invoice.id
        ? {
            ...existingInvoice,
            status: 'delivered',
            goodsReceivingNote: details.goodsReceivingNote || existingInvoice.goodsReceivingNote,
            deliveryNote: details.deliveryNote || existingInvoice.deliveryNote,
            receivingLocation: details.receivingLocation || existingInvoice.receivingLocation,
            receivingNotes: details.receivingNotes || existingInvoice.receivingNotes,
            orderItems: existingInvoice.orderItems?.map(orderItem => {
              const receivedItem = deliveredItems.find(item => item.productId === orderItem.productId);
              return receivedItem
                ? {
                    ...orderItem,
                    receivedQuantity: receivedItem.quantityReceived,
                    rejectedQuantity: receivedItem.quantityRejected,
                    rejectionReason: receivedItem.rejectionReason || orderItem.rejectionReason,
                    deliveredQuantity: receivedItem.quantityAccepted
                  }
                : orderItem;
            })
          }
    ...
    pushLocalNotification(
      'Stock received',
      deliveredItems.map(item => `${item.productName} +${item.quantityAccepted} pcs posted to inventory`).join(', '),
      `stock-received:${invoice.id}`
    );
    toast.success('Stock received', {
      description: deliveredItems.map(item => `${item.productName} +${item.quantityAccepted}`).join(', ')
    });
  };
  ...
  const closeCashDrawerForShiftEnd = (cashierName?: string) => {
    ...
    const drawerClosingBalance = dayBalance.openingBalance + cashSalesToday;

    const nextDayBalance = {
      ...dayBalance,
      closingBalance: drawerClosingBalance,
      status: 'closed'
    } as DayBalance;

    window.localStorage.setItem(LAST_DRAWER_CLOSING_BALANCE_KEY, String(drawerClosingBalance));
    ...
    toast.success('Cash drawer updated', {
      description: `${cashierName || 'Cashier'} shift ended. Drawer closed at ${formatCurrency(drawerClosingBalance)}.`
    });
  };
  ...
  return (
    <div className="animate-soft-pop h-screen bg-gray-50 flex flex-col">
      <Toaster richColors position="top-right" />
