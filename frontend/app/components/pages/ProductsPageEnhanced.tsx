...
import { Plus, Search, Edit, Trash2, TrendingUp, BarChart3, Ruler, PlusCircle } from 'lucide-react';
...
export interface Product {
  ...
  modelNumber?: string;
  size?: number;
  variantUnit?: { id: number; name: string; symbol: string; type: string };
  buyingPrice: number;
  prices: {
    retail: number;
    wholesale: number;
...
interface ProductsPageEnhancedProps {
  ...
  readOnly?: boolean;
  units?: { id: number; name: string; symbol: string; type: string }[];
  onUnitCreated?: (unit: { name: string; symbol: string; type: string }) => void;
}
...
const mapLiveProduct = (product: POSProduct): Product => {
  ...
  return {
    ...
    size: product.size,
    variantUnit: product.variantUnit,
    buyingPrice,
...
export function ProductsPageEnhanced({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const specificName = (formData.name || '').trim() || (formData.parentProduct && formData.size && unit
      ? `${formData.parentProduct} ${formData.size}${unit.symbol}`
      : buildSpecificProductName(formData));
          ...
          {!readOnly && (
                    ...
                    <div className="flex gap-1">
                      <Select
                        ...
                        onValueChange={(val) => {
                          ...
                          setFormData({ ...formData, variantUnit: unit });
                        }}
                      >
                      ...
                      {onUnitCreated && (
                        <Button type="button" size="icon" variant="outline" className="shrink-0" onClick={() => setIsUnitDialogOpen(true)}>
            ...
            <Dialog open={isUnitDialogOpen} onOpenChange={setIsUnitDialogOpen}>
              <DialogContent className="bg-white border-gray-200 max-w-sm">
                <DialogHeader>
                  <DialogTitle className="text-gray-900">Add New Unit</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  <div>
                    <label className="text-sm font-medium text-gray-600">Name</label>
                    <Input value={newUnit.name} onChange={e => setNewUnit({ ...newUnit, name: e.target.value })} className="bg-white border-gray-300" placeholder="e.g., Kilogram, Piece" />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-600">Symbol</label>
                    <Input value={newUnit.symbol} onChange={e => setNewUnit({ ...newUnit, symbol: e.target.value })} className="bg-white border-gray-300" placeholder="e.g., kg, pcs, ml" />
                  </div>
                  <div>
                  ...
                  <DialogFooter className="flex gap-2 pt-2">
                    ...
                    <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => {
                      ...
                      onUnitCreated?.({ name: newUnit.name.trim(), symbol: newUnit.symbol.trim(), type: newUnit.type });
