...
import { Plus, Search, Edit, Eye, EyeOff, Phone, Mail, MapPin } from 'lucide-react';
...
interface CustomersPageProps {
  ...
  onToggleCustomerActive?: (customerId: number, isActive: boolean) => Promise<void>;
}
...
export function CustomersPage({ customers, completedSales, openAddCustomerSignal = 0, onCustomerCreated, onToggleCustomerActive }: CustomersPageProps) {
                ...
                <TableRow key={customer.id} className="border-gray-200">
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" variant="ghost" className={customer.is_active ? 'text-orange-600 hover:text-orange-300' : 'text-green-600 hover:text-green-300'} onClick={() => onToggleCustomerActive?.(customer.id, !customer.is_active)}>
                        {customer.is_active ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                      </Button>
                    </div>
                  </TableCell>
                </TableRow>
