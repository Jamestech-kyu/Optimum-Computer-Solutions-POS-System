...
export function UsersPage({ users, onCreateUser, onUpdateUser, onDeactivateUser, onDeleteUser, onApproveUser, onRejectUser, currentUserRole }: UsersPageProps) {
  ...
  const handleDeleteUser = async (user: BackendUser) => {
    ...
    try {
      ...
      setFormError(error instanceof Error ? error.message : 'User could not be deleted.');
    }
  };
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      ...
                      {currentUserRole === 'super_admin' && (
                        <Button size="sm" variant="ghost" className="text-red-700 hover:text-red-400" onClick={() => handleDeleteUser(user)}
                          title="Permanently delete user">
                          <Trash2 className="w-4 h-4" />
                        </Button>
                      )}
                      <Button size="sm" variant="ghost" className="text-orange-600 hover:text-orange-300" onClick={() => handleDeactivateUser(user)}
                        disabled={user.role === 'admin' && currentUserRole !== 'super_admin'}
                        title={user.role === 'admin' && currentUserRole !== 'super_admin' ? 'Only a super admin can deactivate another admin' : 'Deactivate user'}>
                        <XCircle className="w-4 h-4" />
                      </Button>
                    </div>
                  </TableCell>
