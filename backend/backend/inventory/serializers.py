...
class GoodsReceivedNoteItemSerializer(serializers.ModelSerializer):
    ...
    po_item_remaining = serializers.SerializerMethodField()

    class Meta:
        model = GoodsReceivedNoteItem
        fields = [
            'id', 'goods_received_note', 'purchase_order_item', 'product',
            'product_name', 'product_sku', 'product_barcode',
            'quantity_ordered', 'quantity_received', 'quantity_accepted',
            'quantity_rejected', 'rejection_reason', 'condition', 'condition_notes',
            'unit_cost', 'line_total', 'batch_number',
            'manufacturing_date', 'expiry_date', 'location',
            'is_verified', 'verified_by', 'verified_at', 'notes',
            'po_item_remaining', 'created_at', 'updated_at',
        ]
        read_only_fields = [
            'id', 'goods_received_note', 'product_name', 'product_sku',
            'product_barcode', 'line_total', 'is_verified', 'verified_by',
            'verified_at', 'po_item_remaining', 'created_at', 'updated_at',
        ]
    ...
    def get_po_item_remaining(self, obj):
        return float(obj.purchase_order_item.remaining_to_receive)

    def validate(self, data):
        received = data.get('quantity_received', 0)
        accepted = data.get('quantity_accepted', 0)
        rejected = data.get('quantity_rejected', 0)

        if received < 0:
            raise serializers.ValidationError({"quantity_received": "Quantity received cannot be negative."})
        if accepted < 0:
            raise serializers.ValidationError({"quantity_accepted": "Quantity accepted cannot be negative."})
        if rejected < 0:
            raise serializers.ValidationError({"quantity_rejected": "Quantity rejected cannot be negative."})
...
class GoodsReceivedNoteListSerializer(serializers.ModelSerializer):
    ...
    verified_item_count = serializers.SerializerMethodField()
    created_by_name = serializers.SerializerMethodField()
    status_display = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = [
            ...
            'received_date', 'location', 'status', 'status_display',
            'delivery_note_number', 'courier_name', 'vehicle_number',
            'item_count', 'verified_item_count',
            'created_by', 'created_by_name', 'verified_by', 'verified_at',
            'notes', 'internal_notes', 'created_at', 'updated_at',
        ]
        read_only_fields = [
            'id', 'grn_number', 'location', 'item_count', 'verified_item_count',
            'created_by_name', 'status_display', 'created_at', 'updated_at',
        ]
    ...
    def get_verified_item_count(self, obj):
        return obj.items.filter(is_verified=True).count()

    def get_created_by_name(self, obj):
        return obj.created_by.get_full_name() or obj.created_by.username

    def get_status_display(self, obj):
        return dict(GoodsReceivedNote.STATUS_CHOICES).get(obj.status, obj.status)


class GoodsReceivedNoteDetailSerializer(serializers.ModelSerializer):
    supplier_name = serializers.SerializerMethodField()
    po_number = serializers.SerializerMethodField()
    created_by_name = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = [
            ...
            'verified_at', 'notes', 'internal_notes',
            'created_at', 'updated_at',
        ]
        read_only_fields = [
            'id', 'grn_number', 'purchase_order', 'supplier', 'location', 'items', 'created_by_name',
            'verified_by_name', 'status_display', 'created_at', 'updated_at',
        ]
...
class GoodsReceivedNoteCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = GoodsReceivedNote
        fields = [
            'purchase_order', 'supplier', 'received_date', 'location',
            'delivery_note_number', 'courier_name', 'vehicle_number',
            'notes', 'internal_notes', 'items',
        ]

    def validate_purchase_order(self, value):
        if value.status in ['draft', 'cancelled']:
            raise serializers.ValidationError(
                f"Cannot receive goods for a {value.status} purchase order."
            )
        if value.status == 'completed':
            raise serializers.ValidationError(
                "Purchase order is already fully received and completed."
            )
        return value
    ...
    def validate(self, data):
        ...
        po_items = {str(item.id): item for item in po.items.all()}

        for item_data in data['items']:
            po_item_id = str(item_data['purchase_order_item'].id)
            po_item = po_items.get(po_item_id)
            if not po_item:
                raise serializers.ValidationError(
                    f"Item {po_item_id} is not part of PO {po.po_number}."
                )

            remaining = float(po_item.remaining_to_receive)
            quantity_received = float(item_data.get('quantity_received', 0))
            if quantity_received > remaining:
                raise serializers.ValidationError(
                    f"Cannot receive {quantity_received} for {item_data.get('product_name', 'item')}. "
                    f"Remaining to receive: {remaining}."
                )
    ...
    def create(self, validated_data):
        ...
        for item_data in items_data:
            ...
            grn_item = GoodsReceivedNoteItem(
                ...
                unit_cost=item_data.get('unit_cost', po_item.unit_cost),
                batch_number=item_data.get('batch_number', ''),
                manufacturing_date=item_data.get('manufacturing_date'),
                expiry_date=item_data.get('expiry_date'),
                location=item_data.get('location', validated_data.get('location', 'Main Store')),
                notes=item_data.get('notes', ''),
            )
            grn_items.append(grn_item)

        GoodsReceivedNoteItem.objects.bulk_create(grn_items)
