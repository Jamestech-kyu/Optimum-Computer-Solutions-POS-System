...
class GoodsReceivedNote(models.Model):
    """
    Goods Received Note - documents receipt of goods from a supplier against a PO.
    Supports partial receipts, quality verification, and auto-stock posting.
    """
    ...
    created_by = models.ForeignKey('users.User', on_delete=models.PROTECT, related_name='created_grns')
    verified_by = models.ForeignKey('users.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='verified_grns')
    ...
    @transaction.atomic
    def verify_all_items(self, user):
        """Verify all unverified items in this GRN, post stock, and create stock movements."""
        unverified = self.items.filter(is_verified=False)
    ...
    def cancel(self, user):
        """Cancel the GRN - only possible if no items have been verified."""
        if self.items.filter(is_verified=True).exists():
            raise ValidationError("Cannot cancel GRN with verified items. Reverse stock movements first.")
...
class GoodsReceivedNoteItem(models.Model):
    ...
    Links to the PO item, tracks received/accepted/rejected quantities,
    and handles verification which posts stock.
    """
    ...
    quantity_ordered = models.DecimalField(max_digits=12, decimal_places=2, validators=[MinValueValidator(Decimal('0.01'))])
    quantity_received = models.DecimalField(max_digits=12, decimal_places=2, validators=[MinValueValidator(0)])
    quantity_accepted = models.DecimalField(max_digits=12, decimal_places=2, default=0, validators=[MinValueValidator(0)])
    quantity_rejected = models.DecimalField(max_digits=12, decimal_places=2, default=0, validators=[MinValueValidator(0)])
    ...
    verified_by = models.ForeignKey('users.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='verified_grn_items')
    verified_at = models.DateTimeField(null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        self.line_total = self.unit_cost * self.quantity_accepted
        super().save(*args, **kwargs)

    @transaction.atomic
    def verify(self, user):
        """
        Verify this GRN item:
        1. Update PO item quantity_received
        2. Update product stock (add quantity_accepted)
        3. Create batch if batch_number provided
        4. Create StockMovement record
        5. Create InventoryAlert for rejected/damaged items
        6. Mark as verified
        """
        if self.is_verified:
            raise ValidationError(f"Item {self.product.name} is already verified.")

        product = self.product
        po_item = self.purchase_order_item

        # Update PO item received quantity
        po_item.quantity_received += self.quantity_accepted
        po_item.save(update_fields=['quantity_received'])

        # Update product stock
        stock_before = product.stock_quantity
        product.stock_quantity += self.quantity_accepted
        product.save(update_fields=['stock_quantity', 'updated_at'])
        ...
        if self.batch_number:
            batch, _ = Batch.objects.get_or_create(
                ...
                defaults={
                    ...
                    'remaining_quantity': self.quantity_accepted,
                    'manufacturing_date': self.manufacturing_date,
                    'expiry_date': self.expiry_date,
                    'purchase_order': self.goods_received_note.purchase_order,
                    'purchase_price': self.unit_cost,
                    'supplier': self.goods_received_note.supplier,
                    'location': self.location or self.goods_received_note.location,
                    'notes': f"From GRN: {self.goods_received_note.grn_number}",
                }
            )
        ...
        StockMovement.objects.create(
            ...
            reference_id=self.goods_received_note.grn_number,
            reference_type='grn',
            reason=f"GRN verified: {self.goods_received_note.grn_number}",
            notes=f"Condition: {self.get_condition_display()}. {self.condition_notes}",
            recorded_by=user,
            location=self.location or self.goods_received_note.location,
        )

        # Create alert for rejected items
        if self.quantity_rejected > 0:
            InventoryAlert.objects.create(
                ...
                product=product,
                batch=batch,
                message=f"{product.name}: {self.quantity_rejected} units rejected during GRN {self.goods_received_note.grn_number}. Reason: {self.rejection_reason or self.get_condition_display()}",
                suggested_action="Contact supplier for replacement or credit note.",
            )
        ...
        verified_count = grn.items.filter(is_verified=True).count()
        if verified_count >= total:
            grn.status = 'verified'
            grn.verified_by = user
            grn.verified_at = timezone.now()
            grn.save(update_fields=['status', 'verified_by', 'verified_at', 'updated_at'])
        else:
            grn.status = 'partially_verified'
            grn.save(update_fields=['status', 'updated_at'])

        # Update PO status
        po = grn.purchase_order
        if all(po_item.remaining_to_receive <= 0 for po_item in po.items.all()):
            po.status = 'completed'
        else:
            po.status = 'received'
