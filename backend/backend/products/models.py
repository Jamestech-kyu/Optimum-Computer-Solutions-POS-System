...
class Product(models.Model):
    ...
    # Variant measurement (dynamic unit support)
    ...
    def save(self, *args, **kwargs):
        # Auto-generate display name from generic_name + size + unit
        if self.size is not None and self.variant_unit_id:
            base = self.generic_name or self.name
            symbol = self.variant_unit.symbol
            self.name = f"{base} {self.size}{symbol}"
        if not self.sku:
