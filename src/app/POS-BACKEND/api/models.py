...
class SupplierOrderInvoice(models.Model):
    STATUS_CHOICES = (
        ('paid', 'Paid'),
        ('pending', 'Pending'),
        ('overdue', 'Overdue'),
    ...
    supplier = models.ForeignKey('products.Supplier', on_delete=models.SET_NULL, null=True, blank=True, related_name='order_invoices')
    ...
    payment_method = models.CharField(max_length=40, default='Credit')
    created_at = models.DateTimeField(auto_now_add=True)
    is_active = models.BooleanField(default=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.invoice_number:
            self.invoice_number = f'SUP-INV-{self.id:03d}'
            super().save(update_fields=['invoice_number'])

    def __str__(self):
        return self.invoice_number or self.supplier_name
