...
from .serializers import AppSettingSerializer, ApprovalRequestSerializer, BranchSerializer, OfflineSyncLogSerializer, SupplierOrderInvoiceSerializer, TerminalSerializer
...
def api_index(request):
    return Response({
        ...
        'auth': 'Protected endpoints require a JWT Bearer token from /api/accounts/login/.',
        'endpoints': {
...
def supplier_order_invoices(request):
    if request.method == 'GET':
        queryset = SupplierOrderInvoice.objects.filter(is_active=True).order_by('-order_date', '-id')
        return Response(SupplierOrderInvoiceSerializer(queryset, many=True).data)
    ...
    if supplier_name:
        supplier, _ = Supplier.objects.get_or_create(
            name=supplier_name,
            defaults={'contact_person': request.data.get('contact', '')},
        )

    serializer = SupplierOrderInvoiceSerializer(data={
        **request.data,
...
def app_settings(request):
    setting, _created = AppSetting.objects.get_or_create(
        key='default',
        defaults={'value': {}}
    )
    ...
    value = request.data.get('value', request.data)
    if not isinstance(value, dict):
        return Response({
            'success': False,
            'message': 'Settings value must be an object.'
        }, status=status.HTTP_400_BAD_REQUEST)

    setting.value = value
    setting.updated_by = getattr(request.user, 'username', '') if request.user.is_authenticated else 'frontend'
