...
def add_product(request):
    ...
    base_unit_name = data.pop('base_unit_name', '')

    if isinstance(category_name, list):
        category_name = category_name[0] if category_name else ''
    if isinstance(base_unit_name, list):
        base_unit_name = base_unit_name[0] if base_unit_name else ''
    ...
    if base_unit_name:
        base_unit, _created = ProductUnit.objects.get_or_create(
            name=base_unit_name,
            defaults={'symbol': base_unit_name}
        )
        data['base_unit'] = base_unit.id
