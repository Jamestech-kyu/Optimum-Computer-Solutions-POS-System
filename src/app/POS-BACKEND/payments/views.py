...
class MpesaTransactionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def stk_push(self, request):
        ...
        customer_name = request.data.get('customer_name', 'Optimum POS Customer').strip()
        account_reference = request.data.get('account_reference', 'Optimum POS').strip()
        ...
        if settings.MPESA_DEMO_MODE:
            ...
            mpesa_response = send_stk_push(
                ...
                amount=amount,
                account_reference=account_reference,
                transaction_description='Optimum POS',
            )
...
def mpesa_callback_config(request):
    ...
    return Response({
        ...
        'message': warning or 'Callback URL is suitable for Optimum POS.',
    })
...
def mpesa_health(request):
    try:
        ...
        token_ok = bool(token)
        message = 'Optimum POS M-Pesa access token generated successfully'
